<template>
  <header class="header" id="header" v-click-outside="closeMobileNavbar" v-handle-scroll="closeMobileNavbar">
        <router-link
            ...
            class="logo"
        >
        <img src="../assets/dude1Icon.svg" alt="dude icon"/>
        </router-link>
        <div @click="mobileNavOpen = !mobileNavOpen" class="btn-hamburger" :class="{'btn-hamburger-active': mobileNavOpen}">
      ...
      <ul v-if="user">
            <li class="navbar-user" v-click-outside="closeUserDropDown">
                <a @click.prevent="userDropdownOpen = !userDropdownOpen">
                    <img class="avatar-small" :src="user.avatar" :alt="`${user.name} avatar image`">
                    <span>
                        {{user.name}}
